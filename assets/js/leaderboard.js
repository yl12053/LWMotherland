...
class Leaderboard extends React.Component {
    ...
    let dt = this.getData
    function f(){
      if (!window.hasOwnProperty("datUpload")){
        setTimeout(f, 1);
      } else {
					...
					{this.state.leaders ? (
						this.state.leaders.map((el, i) => (
							...
							>
								...
								<div style={{ animationDelay: 0.3 + 0.1 * i +'s' }} className="leader-bar">
						...
						<div className="empty">Empty record</div>
					)}
				</div>
			</div>
		);
...
ReactDOM.render(React.createElement(window.Leaderboard, null), document.getElementById('app'));