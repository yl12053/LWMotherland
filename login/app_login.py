...
def redr():
  ...
  return redirect(
    f"https://accounts.google.com/o/oauth2/v2/auth?prompt=select_account&scope=https%3A//www.googleapis.com/auth/userinfo.email%20https%3A//www.googleapis.com/auth/userinfo.profile&response_type=token&state={ptoken}&redirect_uri={parse.quote(request.form.get('origin')+'/login_retr')}&client_id={parse.quote(client_id)}",
    301)
...
def test():
  if current_user is None:
    ...
    return "Class: %s<br>Class Number: %s" % (current_user.clas,
                                              current_user.cno)
